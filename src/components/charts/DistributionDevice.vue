<template>
  <StatisticArea title="应用场景北斗设备分布">
...
const deviceType = computed(()=>{
  return props.data.map((item:List)=>{
    return {
      ...
      devicePercent:item.percent
    }
  })
})

const options = {
  color: ['#0268EE', '#02FFFF', '#02FFCD', '#ffe000', '#06D2FF', '#FFBB55', '#0198DA'],
  graphic: {
    elements: [{
      type: 'image',
      style: {
  ...
  legend: {
    ...
    top: 50,
    left: 230,
    data: deviceType.value.map(item => item.deviceName),
    textStyle: {
      color: '#fff',
      fontSize: '12px'
    },
  ...
  series: [{
    ...
    data: deviceType.value.map((item, key) => {
      return [
        {
          value: item.devicePercent,
          ...
          label: {
            ...
            color: '#fff',
            fontSize: '16px',
            formatter: function (name: { name: string, value: number, data: { deviceCount: number } }) {
              return `{name|${name.name}}\n{num|${name.value}%}\n{num|${name.data.deviceCount}}台`
            },
            rich: {
              name: {
                fontSize: 16
...
onMounted(() => {
  ...
  const {resume, pause} = useIntervalFn(() => {
    ...
    dataIndex = dataIndex > deviceType.value.length * 2 ? 0 : dataIndex + 1
  }, 2000)
  resume()
...
const downPlay = () => {
  for (let i = 0; i < deviceType.value.length * 2; i++) {
