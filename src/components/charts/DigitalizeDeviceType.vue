<template>
  <StatisticArea title="数字化施工设备类型分布">
...
const options = {
  ...
  legend: {
    ...
    top: 50,
    left: 230,
    data: props.data.map(item => item.deviceTypeName),
    textStyle: {
      color: '#fff',
      fontSize: '12px'
    },
...
onMounted(() => {
  ...
  const {resume, pause} = useIntervalFn(() => {
    ...
    dataIndex = dataIndex > props.data.length * 2 ? 0 : dataIndex + 1
  }, 2000)
  resume()
