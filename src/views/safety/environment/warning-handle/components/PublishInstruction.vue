<template>
  <el-dialog v-model="useVisible" title="发布指令" width="500">
    <el-form :rules="rules" ref="formRef" :model="form">
      <el-form-item prop="isEvacuate" label="是否撤离" required label-width="120px">
        <el-radio-group v-model="form.isEvacuate">
...
import {getApproveUserList, publishInstruction, workAreaList} from '@/api/safety'
import type {
...
const form = reactive<publishInstructionParams>({
  content: '',
  noticeFrequency: 0,
  noticeNum: 0,
  isEvacuate: 2,
...
const onSubmit = async () => {
  await formRef.value?.validate(async (valid) => {
    if (valid) {
      const {code, message} = await publishInstruction({...form, workAreaId: props.workAreaId})
      if (code === 200) {
